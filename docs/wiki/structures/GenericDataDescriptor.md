---
title: GenericDataDescriptor
original: https://code.google.com/p/unfinnigan/wiki/GenericDataDescriptor
updated: 2011-03-28 07:29:10
...
**GenericDataDescriptor** stores the information on the type, size and name of a data element in a stream of [GenericData](GenericData.md) records. It is the only structural element in [GenericDataHeader](GenericDataHeader.md).
...
> **Note**: All Thermo software runs in Windows on Intel PCs. Because of that, many instances of text strings found in Finnigan files are simply dumps of their internal representations in live Windows software. These strings objects use 2 bytes per character and they start with a length prefix. Here is an example of the [PascalStringWin32](PascalStringWin32.md) encoding of the string '`AGC:`'
>
> ```
> 04  00  00  00 'A' 00 'G' 00 'C' 00 ':' 00  00
> ```
>
> Conversely, the Thermo software simply copies these labels from the data files and uses them to display the data in the GUI.
...
> `\*` This zero-length object only exists in the header and serves as a means of grouping of the data elements, primarily for rendering in the GUI

> `\*\*` I do not know the difference between these two types. One of them may represent a set of bit flags, rather than a single entity

> `\*\*\*` These strings can occupy more storage in the file than their length requires; the surplus storge is usually padded with zeroes, so depending on the mechanism used to read shuch strings, they may need to be truncated to the first trailing zero.
