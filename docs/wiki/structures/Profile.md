---
title: Profile
original: https://code.google.com/p/unfinnigan/wiki/Profile
updated: 2011-03-30 07:40:45
author: selko...@gmail.com
source: Internet Archive copy of Google Code wiki
---

This structure encodes the output of the Fourier transform-based mass analysers and can (possibly) be used to encode the time-domain transient as well. Each scan done in the *profile mode* generates one Profile structure; multiple scans are stored in a continuous stream of [ScanDataPacket](ScanDataPacket.md) structures, in which Profile is an optional element (that is, a data packet may contain no profile -- as is the case of the *centroid-mode* scans).

The spectra can be encoded either in the frequency domain or in the M/z domain. The scans encoded as frequency spectra are accompanied by a set of conversion coefficients (to be found in the corresponding [ScanEvent](ScanEvent.md) and [ScanParameters](ScanParameters.md) records, each containing a copy). These coefficients determine the transform converting the frequency values into the M/z values.
...
![](../images/profile-chart-1.png)

The following is the dump of the [ScanEvent](ScanEvent.md) structure corresponding to scan number 1 (the second [ScanEvent](ScanEvent.md) is, in this case, identical):

`uf-trailer -drw -n 1 LTQ-FTsampleDataSet1.RAW`

| offset | size | type | key | value |
| --- | --- | --- | --- | --- |
| 0 | 80 | [ScanEventPreamble](ScanEventPreamble.md) | `preamble` | `FTMS + p ESI Full ms` |
| 80 | 4 | UInt32 | `np` | `0` |
| 84 | 4 | UInt32 | `unknown long[1]` | `1` |
| 88 | 16 | [FractionCollector](FractionCollector.md) | `fraction collector` | `[300.00-1500.00]` |
...
> ![](../images/profile-formula-1.png)

> **Note**: To be precise, I was only able to verify this formula with respect to B and C, because A was either 0 or an infinitesimal number in all files I could lay my hands on.

## Example 2: LTQ Orbitrap, FTMS

The Orbitrap profiles are almost identical to those obtained with the [LTQ-FT](#Example_1:_LTQ-FT,_FTMS). The only novelty here is that every chunk in the compressed Orbitrap profiles includes a small number, whose meaning remains unknown.
...
The following is the dumps of the two [ScanEvent](ScanEvent.md) structures corresponding to scans 1 and 2 above:

`uf-trailer -drw -n 1 20070522_NH_Orbi2_HelaEpo_05.RAW`

| offset | size | type | key | value |
| --- | --- | --- | --- | --- |
| 0 | 120 | [ScanEventPreamble](ScanEventPreamble.md) | `preamble` | `FTMS + p NSI Full ms` |
| 120 | 4 | UInt32 | `np` | `0` |
| 124 | 4 | UInt32 | `unknown long[1]` | `1` |
| 128 | 16 | [FractionCollector](FractionCollector.md) | `fraction collector` | `[300.00-1700.00]` |
...
> **Note:** In one experiment, a colleague has shown that the conversion parameter B absorbs the variation caused by the run-time lock mass correction.
...
> ![](../images/profile-formula-2.png)

> **Note**: As in the case of the LTQ-FT data, The formula could only be verified with respect to B and C, because A was consistently set to 0 in all availbale data files.

It is interesting to observe that the second pair of coefficients, C and D, apparently works just as well:

> ![](../images/profile-formula-3.png)
