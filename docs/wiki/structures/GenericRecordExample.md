---
title: GenericRecordExample
original: https://code.google.com/p/unfinnigan/wiki/GenericRecordExample
...
> Note that the last descriptor in the instance of [GenericDataHeader](GenericDataHeader.md) shown here (`entry[16]`) contains a null heading. Apparently, its purpose is to introduce a gap in parameter listings rendered in the GUI applications.

## [GenericRecord](GenericRecord.md)

When the above header is applied to the stream of ScanHeader's in the same file, it reveals the following data (also shown within the entire file):
