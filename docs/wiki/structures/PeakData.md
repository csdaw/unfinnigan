---
title: PeakData
original: https://code.google.com/p/unfinnigan/wiki/PeakData
updated: 2010-03-05 14:17:27
...
Calculating the total size is not necessary in normal use (*i.e.*, while converting complete streams), because each `CalledPeak` is referenced via an offset and a count from a corresponding record in `ScanData`, but it is useful in making exploration tools, such as Hachoir parsers, which must cope with random access to all elements.
...
> **Note**: The decoded intensites are stored and passed around as floating point numbers. *Qualbrowser* adds a decimal point and a 0 when it displays them, making them look suspicious, but that is to be expected because they originate as 24-bit integers. I am not familiar with the detector whose designation in this example is `Finnigan MAT LCQ`, but it appears that it either has a 24-bit ADC or an array of 24-bit counters -- otherwise, I don't know how to explain it.
...
(Binary chunks can be easily saved using `hashoir-wx`: right-click on the object and select *Dump to Disk*)
